/**
 * AI Support Agent Swarm — dApp Upgrade Wiring
...
(function() {
  ...
  async function apiFetch(path, opts = {}) {
    try {
      ...
      console.error('[AI Support Agent Swarm] API error:', e);
  ...
  window.dappExportJSON = function(data, filename) {
    ...
    a.href = url;
    a.download = filename || '13-support-agent-export.json';
    a.click();
  ...
  window.dappExportCSV = function(rows, filename) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = filename || '13-support-agent-export.csv';
    a.click();
    URL.revokeObjectURL(url);
